...
// ЗАМЕНИТЕ ЭТУ СТРОКУ НА ВАШ REAL CLIENT ID ИЗ GOOGLE CLOUD CONSOLE
...
window.onload = function() {
    ...
    loadGoogleScript(); // Загружаем скрипт Google Sign-In
};

// Подгрузка официальной библиотеки Google
...
// Инициализация кнопки Google в вашем блоке авторизации
function initGoogleButton() {
    if (typeof google === 'undefined') return;
    
    let container = document.getElementById('google-signin-container');
    if (!container) {
        // Если контейнера для кнопки еще нет в HTML, создаем его под формой входа
        let authBox = document.getElementById('user-auth-box');
        if (authBox) {
            let googleDiv = document.createElement('div');
            googleDiv.id = 'google-signin-container';
            googleDiv.style.cssText = 'margin-top: 15px; display: flex; justify-content: center;';
            authBox.appendChild(googleDiv);
        }
    }
    ...
    try {
        ...
        let targetContainer = document.getElementById('google-signin-container');
        if (targetContainer) {
        ...
        console.log("Google Auth Init Error:", e);
    }
}

// Обработка успешного входа через Gmail
function handleGoogleCredentialResponse(response) {
    if (!response.credential) return;

    // Декодируем JWT токен, который пришел от Google, чтобы узнать почту/имя пользователя
    let responsePayload = parseJwt(response.credential);
    let gmailUser = responsePayload.email; // Например: user@gmail.com

    if (gmailUser) {
        currentUser = gmailUser;
        localStorage.setItem('mta_current_user', gmailUser);
        
        // Сохраняем пользователя в общую базу сайта, если его там еще не было
        let users = JSON.parse(localStorage.getItem('mta_site_users') || '{}');
        if (!users[gmailUser]) {
            users[gmailUser] = 'GOOGLE_OAUTH_USER';
            localStorage.setItem('mta_site_users', JSON.stringify(users));
        }

        checkUserSession();
        alert(`Успешный вход через Gmail (${gmailUser})!`);
        renderPurchasedGoods();
        checkMaintenanceMode();
    }
}

// Вспомогательная функция для расшифровки данных от Google
...
function checkUserSession() {
    ...
    if (currentUser) {
        ...
        // Перерисовываем кнопку Google при выходе из аккаунта
